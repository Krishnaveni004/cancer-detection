...
class DiagnoseMammogramImage(View):
    ...
    def post(self, request):
        ...
        classes = {0: 'Negative', 1: 'Benign Calcification', 2: 'Benign Mass', 3: 'Malignant Calcification',
                   4: 'Malignant Mass'}
...
class DiagnoseLungXRay(View):
    ...
    def post(self, request):
        ...
        with tf.Session() as sess:
            ...
            np.random.seed(1)
            tf.set_random_seed(2)
            img = np.array(Image.open('media/' + filename).resize((224, 224)).convert('RGB').getdata()).reshape(224,
                                                                                                                224, 3)
            data = (1. / 255) * img
